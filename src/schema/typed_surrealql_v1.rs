...
use serde_json::json;
use surrealdb::sql::{Value, Object, Array, Strand, Thing, Id};
...
pub fn encode(v: Value, root: bool) -> Result<Value, Failure> {
	if root {
		let mut encoded: BTreeMap<String, Value> = BTreeMap::new();
		encoded.insert("$schema".to_string(), Value::Strand("https://surrealdb.com/schema/typed-surrealql-v1.json".into()));
